...
1.  **`packages/sdk`**: The core logic library. It handles:
    ...
    - _Goal:_ This package should be usable in both Browser (Web UI) and Node.js (CLI) environments.
...
- **Encryption:** Native `Web Crypto API` (Browser) and `node:crypto` (Node.js).
  - _Constraint:_ Minimize external crypto dependencies to ensure auditability and reduce supply chain risk.
...
- _Note:_ The MK is _never_ sent to the server.
...
- **Key Derivation:** The 256-bit Master Key is split or stretched (depending on specific Bitwarden version behavior) to generate Encryption and MAC keys. _Investigation needed during implementation to match exact Key expansion logic._
...
4.  **Web UI** sends _encrypted_ blob to API.
...
3.  **ApiService:** Implement a **stateless** API client.
    - The SDK does _not_ store tokens internally between instantiations.
