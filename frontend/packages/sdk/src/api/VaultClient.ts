...
export class VaultClient {
  ...
  async userLoginWith2fa(request: TwoFactorLoginRequest): Promise<LoginResponse> {
    return this.request<LoginResponse>('/api/v1/auth/login/user/2fa', 'POST', request);
  }

  async agentLogin(request: AgentLoginRequest): Promise<LoginResponse> {
    return this.request<LoginResponse>('/api/v1/auth/login/agent', 'POST', request);
  }
  ...
  async forgotPassword(request: ForgotPasswordRequest): Promise<{ resetToken: string }> {
    return this.request<{ resetToken: string }>('/api/v1/auth/forgot-password', 'POST', request);
  }
  ...
  // Secrets
  async createSecret(request: CreateSecretRequest, idempotencyKey?: string): Promise<SecretMetadataResponse> {
    return this.request<SecretMetadataResponse>('/api/v1/secrets', 'POST', request, idempotencyKey);
  }
  ...
  async updateSecret(id: string, request: UpdateSecretRequest, idempotencyKey?: string): Promise<SecretMetadataResponse> {
    return this.request<SecretMetadataResponse>(`/api/v1/secrets/${id}`, 'PATCH', request, idempotencyKey);
  }
  ...
  async searchSecrets(request: SearchSecretRequest): Promise<SecretMetadataResponse[]> {
    return this.request<SecretMetadataResponse[]>('/api/v1/secrets/search', 'POST', request);
  }
  ...
  async createAgent(request: CreateAgentRequest, idempotencyKey?: string): Promise<AgentTokenResponse> {
    return this.request<AgentTokenResponse>('/api/v1/agents', 'POST', request, idempotencyKey);
  }

  async rotateAgentToken(id: string, idempotencyKey?: string): Promise<AgentTokenResponse> {
    return this.request<AgentTokenResponse>(`/api/v1/agents/${id}/rotate`, 'POST', {}, idempotencyKey);
  }
  ...
  // Leases
  async listLeases(secretId: string, agentId?: string): Promise<LeaseResponse[]> {
  ...
  async createLease(secretId: string, request: CreateLeaseRequest, idempotencyKey?: string): Promise<void> {
    await this.request<void>(`/api/v1/secrets/${secretId}/leases`, 'POST', request, idempotencyKey);
  }

  async revokeLease(secretId: string, agentId: string): Promise<void> {
    await this.request<void>(`/api/v1/secrets/${secretId}/leases/${agentId}`, 'DELETE');
  }
